...
from knowledge_graph_builder.repo_analyzer import RepoAnalyzer
...
def cmd_pipeline(args: argparse.Namespace) -> None:
    ...
    ain_js_version = getattr(args, 'ain_js_version', '^1.14.0')
    scaffolder = Scaffolder(kg, courses, enable_blockchain=args.enable_blockchain, ain_js_version=ain_js_version)
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    if not output.is_absolute():
        # Make relative paths relative to the package directory, not cwd
...
def cmd_extract(args: argparse.Namespace) -> None:
    ...
    if not analysis_path.is_absolute():
        analysis_path = Path(__file__).parent / analysis_path
...
def cmd_scaffold(args: argparse.Namespace) -> None:
    ...
    from knowledge_graph_builder.models import Course
    courses_path = Path(args.courses)
    if not courses_path.is_absolute():
        courses_path = Path(__file__).parent / courses_path
...
def main() -> None:
    ...
    p_pipeline.add_argument("--model", default="/data/models/gemma-3-27b-it", help="LLM model name on vLLM server")
    ...
    p_pipeline.add_argument("--ain-js-version", default="^1.14.0", help="ain-js npm version to install (default: ^1.14.0)")
    ...
    p_extract.add_argument("--model", default="/data/models/gemma-3-27b-it", help="LLM model name on vLLM server")
    ...
    p_build.add_argument("--model", default="/data/models/gemma-3-27b-it", help="LLM model name on vLLM server")
    ...
    p_scaffold.add_argument("--ain-js-version", default="^1.14.0", help="ain-js npm version to install (default: ^1.14.0)")
