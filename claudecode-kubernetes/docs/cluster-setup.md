...
DELCOM2 서버에 k3s 단일 노드 클러스터를 구축하는 전체 과정을 기록한 runbook.
...
| 항목 | 값 |
|------|-----|
| 서버 | DELCOM2 (Dell PowerEdge T640) |
| ESXi IP | 192.168.1.32 |
...
```bash
ping 192.168.1.32   # DELCOM2 (ESXi)
ping 192.168.1.244  # T640
ping 192.168.1.57   # T550
ping 192.168.1.31   # DELCOM1 (사용 불가 — VM 22개, RAM 91%)
```

### ESXi 웹 UI 접근

브라우저에서 `https://192.168.1.32/ui` 접속 → root 로그인

### 서버 현황 조사 결과

| 서버 | IP | VM 수 | RAM 사용 | 판정 |
|------|-----|-------|---------|------|
| DELCOM1 | 192.168.1.31 | 22개 | 91% | 사용 불가 (회사 서비스) |
| **DELCOM2** | **192.168.1.32** | **14개** | **44%** | **K8s용 — VM 삭제 승인 완료** |
| T640 | 192.168.1.244 | 2개 | 미확인 | 향후 Worker 확장용 (T4 GPU × 4) |
| T550 | 192.168.1.57 | 4개 | 미확인 | 향후 Worker 확장용 (A2 GPU × 4) |
...
주요 설정:
- **언어/키보드**: English (기본값)
- **네트워크**: 고정 IP 192.168.1.100/24, 게이트웨이 192.168.1.1, DNS 8.8.8.8
- **스토리지**: LVM 사용, **ubuntu-lv 크기를 최대(~1020G)로 변경** (기본값 100G → 수동 확장 필요)
- **프로필**: username `k8sadm`, hostname `k8s-node`
...
# 서버에 공개키 복사
ssh-copy-id k8sadm@192.168.1.100
```
...
ssh k8sadm@192.168.1.100 "sudo whoami"
# 출력: root
```
...
```bash
ssh k8sadm@192.168.1.100 "curl -sfL https://get.k3s.io | sh -"
```
...
```bash
# 노드 상태 확인
ssh k8sadm@192.168.1.100 "sudo kubectl get nodes"
# NAME       STATUS   ROLES           AGE   VERSION
# k8s-node   Ready    control-plane   66s   v1.34.4+k3s1

# 시스템 Pod 확인
ssh k8sadm@192.168.1.100 "sudo kubectl get pods -A"
...
# kubeconfig 복사 + IP 수정
mkdir -p ~/.kube
ssh k8sadm@192.168.1.100 "sudo cat /etc/rancher/k3s/k3s.yaml" \
  | sed 's/127.0.0.1/192.168.1.100/' > ~/.kube/config-k8s-node
...
```bash
# Mac에서 실행
scp -r claudecode-kubernetes/ k8sadm@192.168.1.100:~/claudecode-kubernetes/
```
...
# 웹 UI 접속 테스트
curl -s http://192.168.1.100:31000/health
# {"status":"ok"} 이면 정상
...
```bash
# Master 노드에서 토큰 확인
ssh k8sadm@192.168.1.100 "sudo cat /var/lib/rancher/k3s/server/node-token"

# Worker 노드에서 join
curl -sfL https://get.k3s.io | K3S_URL=https://192.168.1.100:6443 K3S_TOKEN=<토큰> sh -
```
...
### ESXi 웹 UI 접근 안 됨
- VPN 연결 확인
- `sudo route add -net 192.168.1.0/24 -interface ppp0` 실행 확인
...
- VPN + route 확인
- kubeconfig의 IP가 192.168.1.100인지 확인: `cat ~/.kube/config-k8s-node | grep server`
- 방화벽 확인: `ssh k8sadm@192.168.1.100 "sudo ufw status"` (disabled여야 함)
